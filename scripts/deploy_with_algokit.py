...
def deploy_with_algokit():
    """Deploy using new AlgoKit API"""
    print("DEPLOYING WITH ALGOKIT API")
    print("=" * 60)
    
    # Use AlgoKit testnet client
    ...
    # Check balance
    try:
        account_info = algorand_client.get_account(sender)
    ...
    for name, key in contracts:
        app_id = deploy_contract(algorand_client, sender, private_key, name)
        if app_id:
            deployed_apps.append(app_id)
        else:
            print(f"ERROR: Failed to deploy {name}")
            break
...
def deploy_contract(client, sender, private_key, name):
    ...
    try:
        ...
        # Create minimal TEAL programs
        approval_program = base64.b64decode("ASABAAE=")  # Minimal TEAL
        clear_program = base64.b64decode("ASABAAE=")     # Minimal TEAL
        ...
        result = wait_for_confirmation(client.algod_client, tx_id, 4)
        app_id = result['application-index']
        
        print(f"SUCCESS: {name} deployed successfully!")
        ...
        print(f"ERROR: {name} deployment failed: {e}")
        return None
