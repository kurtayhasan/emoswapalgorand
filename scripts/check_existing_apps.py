#!/usr/bin/env python3
"""
Check existing applications on testnet
"""
...
def create_real_contract_data():
    """Create real contract data"""
    print("\n🔧 Creating real contract data...")
    
    # Get existing apps
    existing_apps = check_existing_applications()
...
def main():
    """Main function"""
    print("🚀 Checking existing applications on testnet...")
    print("=" * 60)
    
    # Check existing contracts
    existing_ids = check_existing_applications()
    
    if existing_ids:
        print(f"\n✅ Found {len(existing_ids)} existing contracts!")
        return create_real_contract_data()
    else:
